...
//   }
// }, 1000);
// Завантажуємо список задач з localStorage
function loadTasks() {
    const tasks = JSON.parse(localStorage.getItem('tasks')) || [];
    const todoList = document.getElementById('todo-list');
    todoList.innerHTML = ''; // очищаємо список перед відображенням нових задач
    tasks.forEach((task, index)=>{
        ...
        li.innerHTML = `
            ...
            ${task.text}
            <button onclick="removeTask(${index})">\u{412}\u{438}\u{434}\u{430}\u{43B}\u{438}\u{442}\u{438}</button>
        `;
...
function addTask() {
    ...
    if (taskText === '') return; // якщо текст порожній, не додаємо задачу
    const tasks = JSON.parse(localStorage.getItem('tasks')) || [];
    tasks.push({
        text: taskText,
        completed: false
...
// Перемикаємо статус задачі (виконана / не виконана)
...
// Завантажуємо список задач при завантаженні сторінки
